...
export type HeroSlide =
  ...
  | { kind: "collection"; slug: string; name: string; description: string | null; image: string | null };

const AUTO_ADVANCE_MS = 6000;
const VISIBLE_THRESHOLD = 0.6;

/**
 * Swipeable collection highlights inside the hero, over the video. Native CSS
 * scroll-snap, no carousel library: the track is a horizontal scroller with
 * mandatory centre snapping, so touch swiping is the browser's own. Arrows
 * from `md`, dots below, ← → when the track has focus. The active index comes
 * from an IntersectionObserver on the slides, so dots and arrows stay in sync
 * after a swipe as well as after a click.
 *
 ...
 *
 * Slide 0 is the hero as it was (hero.* copy, both CTAs); the rest are one
 * per Hub collection, in the order the page passes them. Copy is the Hub's or
 * the dictionary's — nothing here is invented.
 */
  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={0}
        onKeyDown={onKeyDown}
        className="flex snap-x snap-mandatory overflow-x-auto outline-none [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-gold-pale"
      >
        {slides.map((s, i) => (
          <div key={s.kind === "intro" ? "intro" : s.slug} role="group" aria-roledescription="slide" aria-label={t("home", "slideOf", { n: String(i + 1), total: String(count) })} className="flex w-full shrink-0 snap-center items-center px-1">
            {s.kind === "intro" ? (
              <div className="mx-auto w-full max-w-[820px] text-center lg:mx-0 lg:text-left">
              ...
              <div className="grid w-full items-center gap-8 text-center lg:grid-cols-12 lg:text-left">
                <div className={`mx-auto max-w-[720px] lg:mx-0 ${s.image ? "lg:col-span-7" : "lg:col-span-9"}`}>
                  ...
                  {s.description && <p className="mt-5 text-[15px] leading-relaxed text-chalk/80 lg:text-base">{s.description}</p>}
                  <div className="mt-8 flex justify-center lg:justify-start">
                    <Button asChild><Link href={`/collections/${s.slug}`}>{t("home", "slideShop", { name: s.name })}</Link></Button>
                  </div>
                </div>
                {s.image && (
                  <div className="hidden lg:col-span-5 lg:block">
                    <div className="aspect-[4/3] overflow-hidden rounded-sm border border-hairline/40 shadow-lg">
      ...
      {count > 1 && (
        <>
          <button type="button" aria-label={t("home", "slidePrev")} onClick={() => goTo(active - 1)} className={`${arrow} -left-2 lg:-left-14`}>
          ...
          <button type="button" aria-label={t("home", "slideNext")} onClick={() => goTo(active + 1)} className={`${arrow} -right-2 lg:-right-14`}>
          ...
          <div className="mt-8 flex justify-center gap-2 lg:justify-start" role="tablist" aria-label={t("home", "slideEyebrow")}>
            {slides.map((s, i) => (
              <button
              ...
              >
                <span className={`block h-2 w-2 rounded-full transition-colors ${i === active ? "bg-orange" : "bg-chalk/40"}`} />
              </button>
            ))}
          </div>
        </>
