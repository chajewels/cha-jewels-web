...
export function MobileNav({ links, openLabel, closeLabel, account }: { links: { href: string; label: string }[]; openLabel: string; closeLabel: string; account?: DrawerAccount | null }) {
  ...
  return (
    <>
      ...
      {open && createPortal(
        <div className="fixed inset-x-0 bottom-0 top-[68px] z-30 flex flex-col gap-1 overflow-y-auto bg-charcoal-deep px-[clamp(18px,4vw,48px)] py-8 xl:hidden">
          {links.map((l) => <Link key={l.href} href={l.href} onClick={() => setOpen(false)} className="border-b border-rule-soft py-3 font-display text-3xl text-chalk">{l.label}</Link>)}
          {account && (
            <section aria-label={account.menuLabel} className="mt-8 border-t border-rule pt-6">
              ...
              <ul className="mt-3">
                {account.items.map((it) => (
                  <li key={it.href}><Link href={it.href} onClick={() => setOpen(false)} className="block border-b border-rule-soft py-3 text-lg text-chalk/85 hover:text-gold-pale">{it.label}</Link></li>
                ))}
              </ul>
