...
type Props = { type: "store" } | { type: "product"; product: Product };
export function JsonLd(props: Props) {
  ...
  const data = props.type === "store"
    ...
    : { "@context": "https://schema.org", "@type": "Product", name: props.product.name, sku: props.product.sku, image: primaryImage(props.product)?.url, material: props.product.karat ?? undefined, description: props.product.description_en ?? undefined,
        offers: { "@type": "Offer", price: fromPrice(props.product) ?? undefined, priceCurrency: "JPY", availability: props.product.product_variants.some((v) => v.stock_qty > 0) ? "https://schema.org/InStock" : "https://schema.org/SoldOut", url: `${base}/products/${props.product.slug}` } };
