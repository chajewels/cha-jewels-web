...
Implements Phase 0 and Phase 1 of the implementation plan (foundations, catalog and brand) and the entry points for Phase 3 (live claims). Products are managed in the Cha Jewels Hub; this site reads them from the same Supabase project.

## Setup
1. `cp .env.example .env.local` and fill in the Hub's Supabase URL and anon key.
2. Have Lovable/Cynthia apply `supabase/migrations/0001_website_catalog.sql` and implement `supabase/contracts/README.md` (at minimum `layaway_quote`, or the calculator shows an error state).
3. `npm install && npm run db:types && npm run dev`
...
- `npm run typecheck`
- `npm run db:types` — regenerate Supabase types from the shared project.
