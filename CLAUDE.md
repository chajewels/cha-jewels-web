...
- **Every customer-facing money figure comes from the Hub (owner rule 2026-09-25).** The storefront never computes a percentage or converts a currency. Down payments on the product card and product page are the variant's `down_payment_jpy` / `down_payment_php` (piece alone, owner decision D1), shown as "¥21,894 (₱8,699)" — yen first, peso in brackets (D3, `formatYenPeso`) — English only, and only when **both** fields are present; otherwise no reserve line, never fallback maths. The calculator's ₱ mode is a Hub **peso quote** (`hub.layawayQuote(price_jpy, term, "PHP")`): deposit, monthly, total and the term minimums (`min_amount_php`) are the Hub's pesos; 503 `fx_unavailable` shows the checkout's rate-unavailable message. No `toPhp`, no `phpRate`, no hardcoded rate. **Guard:** `npm run check:money` (CI) fails on a price multiplied by a decimal, arithmetic with a rate or a deposit percentage, `toPhp`/`phpRate`, or a hardcoded `jpy_php` under `app/`, `components/`, `lib/` (`lib/fixtures.ts`, the preview Hub stand-in, excepted).
