...
Status: plan. Commit this file to `cha-jewels-web` at `docs/tasks/phase2-plan.md` and to `la-tracking` at `docs/PHASE2-WEBSITE.md` so both Claude Code sessions read the same source.
...
2. Hub: quotes + transfer orders (`/checkout/quote`, `/checkout/pay` transfer path) on **`cash_orders` / `cash_order_items`**. Site: cart, checkout with transfer only. **First real order end to end without Square.** — **BUILT** (migration `20260911120000`).
