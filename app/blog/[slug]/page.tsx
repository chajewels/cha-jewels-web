  ...
  return (
    <article className="py-[clamp(48px,7vw,96px)]">
      <div className="wrap max-w-[760px]">
        ...
        <time dateTime={p.date} className="mt-6 block text-xs text-chalk/55">{p.date}</time>
        <h1 className="mt-2 text-[clamp(32px,4.5vw,64px)]">{p.title[lang]}</h1>
        <div className="mt-8 space-y-5 text-[17px] leading-relaxed text-chalk/85">{p.body[lang].map((para, i) => <p key={i}>{para}</p>)}</div>
      </div>
    </article>
  );
}
