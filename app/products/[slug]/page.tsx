...
export default async function ProductPage({ params }: { params: Promise<{ slug: string }> }) {
  const [p, region, lang] = await Promise.all([getProductBySlug((await params).slug), getRegion(), getLang()]);
  if (!p) notFound();
  const t = tr(lang);
  const variant = p.product_variants[0];
  const price = region === "PH" ? variant?.price_php ?? variant?.price_jpy : variant?.price_jpy;
  const img = primaryImage(p);
  const desc = lang === "ja" ? p.description_ja ?? p.description_en : p.description_en ?? p.description_ja;
  return (
    <>
      <JsonLd type="product" product={p} region={region} />
        ...
        <div className="wrap grid gap-10 md:grid-cols-2">
          <figure className="border border-gold bg-velvet-deep p-1.5">
            ...
            <dl className="grid grid-cols-3 border-t border-gold bg-velvet-deep">
              <div className="border-r border-rule p-4"><dt className="text-xs text-champagne/55">{t("product", "metal")}</dt><dd className="font-display text-2xl text-gold-pale">{p.karat ?? "—"}</dd></div>
          ...
          <div>
            ...
            {price != null && <PriceBlock price={price} region={region} lang={lang} className="mt-6" />}
            ...
            {price != null && <LayawayCalculator region={region} lang={lang} initialPrice={price} className="mt-8" />}
          </div>
        </div>
      </section>
    </>
