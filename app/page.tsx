        ...
        <div className="wrap grid items-end gap-12 md:grid-cols-2">
          <div>
            ...
            <div className="mt-9 flex flex-wrap gap-3">
              <Button asChild><Link href="/collections/k18-gold">{t("hero", "cta1")}</Link></Button>
              <Button asChild variant="ghost"><Link href="/layaway">{t("hero", "cta2")}</Link></Button>
            </div>
          </div>
          {featured[0] && <ProductCard product={featured[0]} region={region} lang={lang} featured />}
        </div>
      </section>
      ...
      <section className="border-b border-rule-soft py-[clamp(64px,9vw,120px)]">
        <div className="wrap">
          <div className="mb-12 flex flex-wrap items-end justify-between gap-6">
            ...
            <Link className="text-gold-pale underline underline-offset-4" href="/collections/k18-gold">{t("home", "viewAll")}</Link>
          </div>
          <div className="rule-grid grid grid-cols-2 lg:grid-cols-4">{featured.map((p) => <ProductCard key={p.id} product={p} region={region} lang={lang} />)}</div>
        </div>
      </section>
