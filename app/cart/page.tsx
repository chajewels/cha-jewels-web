  ...
  return (
    <section className="surface-light bg-chalk text-charcoal-deep py-[clamp(48px,7vw,96px)]">
      <div className="wrap max-w-[900px]">
        ...
        {dropped > 0 && (
          <p className="mt-6 border border-gold-dark px-4 py-3 text-sm text-gold-dark">{t("cart", "dropped")}</p>
        )}
          ...
          <>
            ...
            <div className="mt-8 border-t border-hairline pt-6">
              <div className="flex items-baseline justify-between">
                <span className="text-charcoal/70">{t("cart", "subtotal")}</span>
                <span className="font-display text-3xl text-gold-dark">{formatMoney(subtotal)}</span>
              </div>
              <p className="mt-2 text-sm text-charcoal/70">{t("cart", "shippingNote")}</p>
