...
- **`Product.short_name_en` / `short_name_ja: string | null`**: the short display name ("Cross Pendant"). Until then, the storefront derives it from a name that starts with its own SKU (`shortName` in `lib/hero-deck.ts`).
