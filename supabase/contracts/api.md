...
| `GET /content/posts?type=` | `Post[]` | **Hub route pending.** Editorial, newest first; `type` is `article\|news` and narrows the list. `Post = { id, slug, type, published_at, cover_url: string\|null, layaway_only: boolean, title_en, title_ja, excerpt_en, excerpt_ja, body_en, body_ja }`. Per-language fields may be null — the site drops a post from a language it has no title AND body in, rather than rendering an empty page. `body_*` is **Markdown**, not HTML: the site renders paragraphs, `##`/`###`, lists, `[text](href)` and bold/italic, escapes everything first and emits no raw HTML under any input. `layaway_only` mirrors the storefront's language rule — such a post is listed and readable on English only. Cached on tag `content`. |
